...
async def notify_new_user(email: str, auth_provider: str) -> None:
    """Notify when a new user registers."""
    provider_label = "Google" if auth_provider == "google" else "Email"
    await notify(f"👤 新規ユーザー登録: {email} ({provider_label})")


async def notify_error(endpoint: str, error: str) -> None:
    """Notify when a server error occurs."""
    await notify(f"🚨 エラー: {endpoint} — {error[:200]}")
