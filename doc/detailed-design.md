...
| モード | 大きさ | 含む内容 |
|--------|--------|---------|
| 高速モード | 約65トークン | 言語判定 + 話者前提固定 + 方向別トーン指示 + 「1行のみ返答」 |
| 詳細モード | 約145トークン | 上記 + 出力フォーマット定義（翻訳→別表現→ポイント） + 解説言語=入力言語ルール |
...
方向別トーン:
- JP→VI: カジュアル（年上男性が年下女性に話す自然な口調）
- VI→JP: です・ます調（年下女性が年上男性に話すので敬語、過度な尊敬語/謙譲語は避ける）
...
- モデル選択ドロップダウン
- ディベートモード切替
- 拡張思考モード切替（翻訳ON時は翻訳の高速/詳細トグルを兼ねる）
- 翻訳モード切替（🌐、添付アイコンの右に配置、セッション単位で localStorage 保存）
- 音声入力（翻訳モードON時のみ表示、MediaRecorder→Geminiマルチモーダル）
- 画像プレビュー表示

入力欄ツールバーのレイアウト:
[📎 添付] [🌐 翻訳] ...spacer... [高速/思考モード] [🎤 マイク（翻訳ON時）] [➤ 送信]
```
