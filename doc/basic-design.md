...
| 機能 | 概要 |
|------|------|
| マルチプロバイダー AI チャット | Claude / GPT / Gemini の11モデルに対応したストリーミングチャット |
...
| 日越翻訳モード | 日本語 ⇄ ベトナム語の自動方向判定翻訳（テキスト/音声、高速/詳細） |
| 音声入力翻訳 | マイク録音 → Gemini マルチモーダルで直接翻訳（1ステップ） |
| 画像添付 | チャットに画像を添付してマルチモーダル対話 |
| チャットエクスポート | Text / PDF 形式で会話を書き出し |
| 多言語対応（i18n） | 日本語・英語・ベトナム語（ブラウザ自動検出） |
...
翻訳トグル ON 時、通常チャットが翻訳特化モードに切り替わる。

| 機能 | 説明 |
|------|------|
| 自動方向判定 | 入力言語を検出し、もう一方の言語に翻訳 |
| 話者前提固定 | 日本語側=年上男性（anh）、ベトナム語側=年下女性（em） |
| 方向別トーン | JP→VI: カジュアル（年上男性→年下女性） / VI→JP: です・ます調（年下女性→年上男性） |
| 高速モード | 翻訳結果1行のみ返答（解説なし、コスト約1/10） |
| 詳細モード | 翻訳＋別表現＋ポイント解説（解説の言語は入力言語に合わせる） |
| 音声入力 | マイク録音した音声を Gemini マルチモーダルに直接送信（中間文字起こしなし） |
| 推奨モデル | Gemini 3.5 Flash（2.5 Pro と同等品質・約35%高速） |
| ツール無効化 | 翻訳モード時はツール/Web検索/ユーザープロンプト/コンテキストを全部無効化 |
| 状態保存 | 翻訳ON/OFF・高速/詳細はセッション単位で localStorage に保存 |
| UI配置 | 翻訳トグル（🌐）は入力欄ツールバー内、添付アイコンの右に配置 |

実装:
- バックエンド: `backend/base_prompt.py` の `_TRANSLATION_PROMPT_FAST` / `_TRANSLATION_PROMPT_DETAILED`
- フロントエンド: `frontend/components/ChatInput.tsx` の翻訳トグル + マイクボタン
- リクエスト: `ChatRequest.translation_mode` / `translation_fast_mode` / `audio` フィールド
...
| 言語切替 | 日本語 / 英語 / ベトナム語（ブラウザの言語設定で自動検出、cookie `locale` で上書き可） |
